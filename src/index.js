    ...
    Array.from(row).forEach((el) => {
      ...
      if (titleSecondClass) {
        localStorage.setItem(
          `${titleSecondClass}${list_detector}`,
          `${titleTxt}`
        )
      }
      if (dueDate && dueDateSecondClass) {
        localStorage.setItem(
          `${dueDateSecondClass}${list_detector}`,
          `${dueDate.value}`
        )
      }
      if (DescriptionSecondClass) {
        localStorage.setItem(
          `${DescriptionSecondClass}${list_detector}`,
          `${DescriptionTxt}`
        )
      }
      if (prioritySecondClass) {
        localStorage.setItem(
          `${prioritySecondClass}${list_detector}`,
          `${priorityTxt}`
        )
      }
    })
...
createListButton.addEventListener('click', () => {
  ...
  newInput.addEventListener('blur', async () => {
    ...
    if (newInputValue !== '') {
      ...
      newlist.addEventListener('click', () => handleListClick(newInputValue), {
        once: false,
      })
...
export { ColorChanging, removeTask, setNewList_detector, handleListClick }
